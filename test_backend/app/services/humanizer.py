...
def _replace_ai_phrases(text, rng):
...
def _remove_template_patterns(text):
    for pattern in AI_SENTENCE_PATTERNS:
        text = re.sub(pattern, '', text, flags=re.MULTILINE)
    return text


def _vary_sentence_endings(text, rng):
...
def _add_colloquial_connectors(text, rng):
...
def _check_sentence_length_variance(text):
...
def _break_uniform_sentences(text, rng):
...
def humanize_text(text, intensity="medium"):
    ...
    _seed = int(_hl.sha256(text[:50].encode("utf-8")).hexdigest()[:8], 16)
