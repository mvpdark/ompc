...
class Settings(BaseSettings):
    ...
    def _warn_weak_jwt_secret(self) -> "Settings":
        weak_secrets = {"replace-with-a-long-random-secret", "change-me-in-production"}
        is_dev_profile = self.runtime_profile.strip().lower() in {
            "desktop",
            "developer",
            "development",
        }
        if self.jwt_secret_key in weak_secrets:
            if not is_dev_profile:
                raise ValueError(
                    "JWT_SECRET_KEY 使用默认弱密钥，非开发环境"
                    f"（runtime_profile={self.runtime_profile}）"
                    "必须在 .env 中替换！"
                )
            warnings.warn(
