...
def get_image(
    ...
    if image.content_id is not None and image.content_id > 0:
    ...
    elif hasattr(image, "created_by") and image.created_by is not None:
        # Orphan image (content_id is None or 0): verify ownership via created_by
        ...
        # Orphan image with no creator attribution: deny access
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="未找到这张封面图。",
        )
