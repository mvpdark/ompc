...
const DEFAULT_OMPC_API_PORT = process.env.NEXT_PUBLIC_OMPC_API_PORT ?? "60011";
const DEFAULT_LOCAL_OMPC_API_BASE = `http://localhost:${DEFAULT_OMPC_API_PORT}/api`;
...
/**
 * 获取 OMPC 壳后端 API base URL（认证、工作台、域管理）。
 * OMPC 壳运行在端口 60010，提供统一的登录认证和工作台管理。
 */
export function getOmpcApiBase() {
  const configuredApiBase = process.env.NEXT_PUBLIC_OMPC_API_BASE_URL;

  if (typeof window === "undefined") {
    return trimTrailingSlash(configuredApiBase ?? DEFAULT_LOCAL_OMPC_API_BASE);
  }
  ...
  return `${protocol}//${hostname || "localhost"}:${DEFAULT_OMPC_API_PORT}/api`;
}
